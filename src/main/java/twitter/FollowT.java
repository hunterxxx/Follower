...
public class FollowT {
	...
	/**
	 * Represents a user that started following another user at some point in
	 * the past.
	 */
	...
	/**
	 * A special follower who, after starting to follow another user, has since
	 * stopped following that user again.
	 */
	 ...
	 * 
	 * @param screenName
	 *            the name of the user for which unfollowers should be returned
	 * @param interval
	 *            number of milliseconds between now and the point in the past
	...
	public List<Unfollower> getRecentUnfollowers(String screenName, long interval) {
		...
		DBCursor dbresult = fhistory
				.find(new BasicDBObject("followee", followee).append("end", new BasicDBObject("$gte", cutoff)))
	 ...
	 * 
	 * @param followee
	 *            the user for which the beginning of time should be returned
	 * @return point in time when the first scan of that user started
	 */
	public Date beginningOfTime(int followee) {
		AggregationOutput agr = fhistory.aggregate(new BasicDBObject("$match", new BasicDBObject("followee", followee)),
				new BasicDBObject("$group", new BasicDBObject("_id", null).append("beginning_of_time",
	...
	/**
	 * Returns the list of users which have started following the given user in
	 * the period between now and <code>interval</code> milliseconds in the
	 * past.
	 * 
	 * @param screenName
	 *            the name of the user for which new followers should be
	 *            returned
	 * @param interval
	 *            number of milliseconds between now and the point in the past
	...
	public List<Follower> getRecentFollowers(String screenName, long interval) {
		...
		DBCursor dbresult = fhistory
				.find(new BasicDBObject("followee", followee).append("start", new BasicDBObject("$gte", cutoff)))
	...
	public int newFollowersOnDay(String screenName, Date d) {
		...
		int result = fhistory.find(new BasicDBObject("followee", followee).append("start",
				new BasicDBObject("$gte", start).append("$lt", end))).count();
		return result;
	}
	...
	public int lostFollowersOnDay(String screenName, Date d) {
		...
		Date start = c.getTime();
		Date end = new Date(c.getTimeInMillis() + 86400000);
		int result = fhistory.find(new BasicDBObject("followee", followee).append("end",
				new BasicDBObject("$gte", start).append("$lt", end))).count();
		return result;
	}
